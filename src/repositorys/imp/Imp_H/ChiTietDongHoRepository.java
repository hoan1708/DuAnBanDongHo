...
//            stmt.setInt(2, maDongHo);
    ...
    public ArrayList<DongHoChiTietViewModel> filterByTenDongHo(String tenDH) {
        ...
        try {
            ...
            while (rs.next()) {
                DongHoChiTietViewModel dongHo = new DongHoChiTietViewModel
                                                (rs.getInt("idChiTietDH"),rs.getString("tenDongHo"),rs.getString("tinhNang"), rs.getInt("thoiGianBaoHanh"),
        ...
        try {
            ...
            while (rs.next()) {
               ...
               // Integer soLuong, String hinhThucKhuyenMai, Double giaGiam) {
                ...
                SanPhamTronHoaDonModel sp = new SanPhamTronHoaDonModel
                    (rs.getInt("idChiTietDH"), rs.getString("tenDongHo"), rs.getString("gioiTinh"), rs.getString("kieuMay"),
    ...
    @Override
    public boolean updateSoLuong(int soLuong, int idChiTietDH) {
        try {
            Class.forName("com.microsoft.sqlserver.jdbc.SQLServerDriver");
            conn = DBConnection.openDbConnection();
            String sql = "update ChiTietDongHo set soLuong = ? where idChiTietDH=?";

            PreparedStatement stmt = conn.prepareStatement(sql);
            stmt.setInt(1, soLuong);
            stmt.setInt(2, idChiTietDH);
            stmt.executeUpdate();
    ...
    @Override
    public boolean updateTrangThaiDH(int soLuong) {
        try {
            Class.forName("com.microsoft.sqlserver.jdbc.SQLServerDriver");
            conn = DBConnection.openDbConnection();
            String sql = "";
            if(soLuong==0){
                sql = "update ChiTietDongHo set idTrangThaiCTDH=2 WHERE soLuong=0";
            }else{
                sql = "update ChiTietDongHo set idTrangThaiCTDH=1 WHERE soLuong>0";
            }
            ...
            System.out.println("Error khi cập nhật trạng thái đồng hồ" + ex.getMessage());
        }
        return false;
    }
